...
bool bvh_node::bounding_box(double time0, time1, aabb& output_box) const {
    output_box = box;
    return true;
}
...
bvh_node::bvh_node( std::vector<shared_ptr<hittable>>& objects,
    ...
    ||  !right->bounding_box(time0, time1, box_right)) {std::cerr << "No bounding box in bvh_node constructor.\n";}
